...
CUDA_VISIBLE_DEVICES=1,2 NCCL_P2P_DISABLE=1 python -m torch.distributed.run --nproc_per_node=2 train_cloome_chembert_contrastive.py \
    ...
    --aggregation-level compound_aggregated \
    --image-size 224 \
    --backbone-lr 5e-5 \
    --downstream-lr 2e-4 \
    --embed-dim 256 \
